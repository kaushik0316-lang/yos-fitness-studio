...
export default function MemberPortalPage() {
  ...
  const [announcements, setAnnouncements] = useState<{ id: string; title: string; body: string; ctaLabel: string }[]>([]);
      ...
      fetch("/api/member/ping", {
        ...
        .then((data) => {
          if (data.member) {
            ...
            setPin(stored);
            setPhase("dashboard");
            fetch(`/api/member/announcements?pkg=${encodeURIComponent(data.member.packageName ?? "")}&status=${data.member.status}`)
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-y-auto" style={{ background: "#0a0a0a" }}>
      ...
      <div className="flex-1 w-full max-w-md mx-auto px-4 py-5 flex flex-col gap-4">
        ...
        {isActive && prog && prog.daysLeft <= 7 && prog.daysLeft > 0 && (
          <div className="rounded-3xl p-5" style={{ background: "rgba(249,115,22,0.07)", border: "1px solid rgba(249,115,22,0.2)" }}>
            <p className="text-orange-400 font-bold text-sm">⚡ Membership expiring in {prog.daysLeft} day{prog.daysLeft === 1 ? "" : "s"}!</p>
        ...
        {/* ── From Yos — announcements ── */}
        {announcements.length > 0 && (
          <div className="space-y-3">
            <p className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "#4b5563" }}>From Yos</p>
            {announcements.map((a) => (
