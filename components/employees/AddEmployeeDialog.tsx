...
export function AddEmployeeDialog({ open, onClose }: Props) {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm({
    defaultValues: {
      ...
      salaryType: "FIXED_MONTHLY", monthlySalary: "", perDaySalary: "", notes: "",
    },
  });
  ...
  async function onSubmit(data: any) {
    ...
    try {
      ...
      toast({ title: "Employee added!", description: `Registered as ${result.employeeId}` });
      handleClose();
    } catch (e: any) {
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
          <div>
            ...
            <input {...register("fullName", { required: true })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
          </div>
