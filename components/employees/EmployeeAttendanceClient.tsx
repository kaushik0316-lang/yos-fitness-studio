...
import { ChevronLeft, ChevronRight, Loader2, Save, CalendarDays, Users } from "lucide-react";
...
type Props = {
  employees: Employee[];        // active only (for attendance grid)
  allEmployees: Employee[];     // active + inactive (for staff tab)
...
export function EmployeeAttendanceClient({ employees, allEmployees, attendanceMap, month, year, userId, userRole }: Props) {
  ...
  const [localMap, setLocalMap] = useState<Record<string, Record<string, string>>>(() =>
    JSON.parse(JSON.stringify(attendanceMap))
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        ...
        >
          ...
          <span className={cn(
            ...
            tab === "staff" ? "bg-orange-100 text-orange-700" : "bg-gray-200 text-gray-500"
          )}>
      ...
      {tab === "attendance" && (
        <>
          ...
          {canEdit && (
            <p className="text-xs text-gray-500">Click a cell to cycle: Present → Absent → Half Day → Weekly Off → Leave → Paid Leave</p>
          )}
          ...
          {employees.length === 0 ? (
            <div className="bg-white rounded-xl border p-12 text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">Go to the <button onClick={() => setTab("staff")} className="text-orange-500 hover:underline font-medium">Staff tab</button> to add your team.</p>
            </div>
          ) : (
