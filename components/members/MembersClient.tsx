  ...
  return (
    <div className="space-y-4">
      ...
      {birthdayMembers.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: "#161616", border: "1px solid rgba(251,191,36,0.2)" }}>
          <div className="flex items-center gap-2 px-5 py-3.5 border-b border-white/5">
            <span className="text-lg">🎂</span>
            <p className="font-bold text-white text-sm">Birthdays This Week</p>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full ml-1"
              style={{ background: "rgba(251,191,36,0.15)", color: "#fbbf24" }}>{birthdayMembers.length}</span>
          </div>
          <div className="divide-y divide-white/5">
            {birthdayMembers.map((m) => {
              ...
              const dobStr = dob.toLocaleDateString("en-IN", { day: "2-digit", month: "short" });
              const phone = (m.whatsapp ?? m.phone ?? "").replace(/\D/g, "").slice(-10);
              const firstName = m.fullName.split(" ")[0];
              const bdayMsg = `🎂 Happy Birthday, ${firstName}! 🎉\n\nWishing you a wonderful day filled with joy and great fitness goals ahead! Keep crushing it — the whole Yos team is cheering for you! 💪\n\n– Team Yos Fitness Studio`;
              return (
                <div key={m.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/[0.02] transition-colors">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-extrabold flex-shrink-0"
                    style={{ background: isToday ? "rgba(251,191,36,0.2)" : "rgba(255,255,255,0.06)", color: isToday ? "#fbbf24" : "#9ca3af" }}>
